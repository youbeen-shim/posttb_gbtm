...
```{r}
patient_demographics <- function(n, grp) {
  ...
  demographics_table <- tibble(
    ...
    trajectory_group = grp  # remove later
  )
...
visit_trajectories <- function(n, grp) {
  ...
  for(i in 1:length(grp)) {
    ...
    if(group == 1) { # exponential decay; high visits -> low
      ...
      spike_quarters <- sample(1:n, size = sample(2:5, 1))
      base_pattern[spike_quarters] <- base_pattern[spike_quarters] + sample(4:8, length(spike_quarters), replace = TRUE)
      noise_sd <- 2.0
...
```{r}
patient_demographics <- patient_demographics(n = num_patients, grp = trajectory)
quarterly_visits <- visit_trajectories(n=num_quarters, grp=trajectory)
```

```{r}
cat("1. PATIENT DEMOGRAPHICS TABLE:\n")
# dim
## row; num patients
